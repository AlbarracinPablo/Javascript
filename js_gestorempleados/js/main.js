$(document).ready(function () {
    ...
    if (window.localStorage.getItem('empleados') === null) {
        $('.empleados').append(`<h1 style="text-align:center;">No hay empleados cargados actualmente ...</h1>`)
    }
    else {
...
function resetApp()
{
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            Swal.fire(
                'Reseteado',
                'La aplicacion ha sido reseteada',
                'success'
                )
                localStorage.clear();
...
$("#addEmpleado").on('submit', function (e) {
    ...
    const sexo = document.querySelector('input[name="flexRadioDefault"]:checked').value;
...
function validar(data) {
    ...
    if (data.nombre == '') {
        control = false;
        $('.errorNombre').empty();
        $('.errorNombre').append('<label for="">EL NOMBRE Y APELLIDO SON OBLIGATORIOS</label>');
    ...
    if (data.sueldo == '') {
        ...
        $('.errorSueldo').empty();
        $('.errorSueldo').append('<label for="">EL SUELDO NO PUEDE SER NEGATIVO</label>');
        $('.errorSueldo').css("background-color", "red");
        $('.errorSueldo').css('visibility', 'visible');
...
guardarEmpleado = (data) => {
    ...
    if (empleados === null) {
        ...
        const getEmpleados = window.localStorage.getItem('empleados');
        const actualArray = JSON.parse(getEmpleados);
